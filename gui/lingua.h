...
inline void installaTraduttori(QApplication& app, QString const& codice)
{
    ...
    // Qt spedisce qt_de.qm, qt_fr.qm... con il solo codice breve.
    QString const breve = codice.left(2);
    QString const dove = QLibraryInfo::path(QLibraryInfo::TranslationsPath);
    if (diQt.load(QStringLiteral("qt_%1").arg(breve), dove)
        || diQt.load(QStringLiteral("qtbase_%1").arg(breve), dove))
